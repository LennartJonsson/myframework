<?php
/**
 * A model for managing MyFrameWork modules.
 *
...
class CMModules extends CObject
{
   ...
   * Properties
   */
  private $MyFrameWorkCoreModules = array('CMyFrameWork', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $MyFrameWorkCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CTextFilter');
  ...
  public function ReadAndAnalyse()
  {
	...
	if(class_exists($module))
	{
	  ...
	  $modules[$module]['isMyFrameWorkCore']   = in_array($rc->name, array('CMyFrameWork', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
	  $modules[$module]['isMyFrameWorkCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
	}
      }
    }
    $dir->close();
  ...
  private function GetDetailsOfModule($module)
  {
    ...
    if(class_exists($module))
    {
      ...
      $details['isMyFrameWorkCore'] = in_array($rc->name, $this->MyFrameWorkCoreModules);
      $details['isMyFrameWorkCMF']  = in_array($rc->name, $this->MyFrameWorkCMFModules);
  ...
  public function Install()
  {
	...
	if($module['isManageable'])
	{
      ...
      writesettingconfigfile();
    }
    else
    {
      $installed['None']['name']    = 'None';
      $installed['None']['result']  = 'Data-directory not writable';
    }
    return $installed;
  }
}
