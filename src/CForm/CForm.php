...
class CFormElement implements ArrayAccess
{
  ...
  public function __construct($name, $attributes=array())
  {
    ...
    $this['name'] = $name;
    if(is_callable('CMyFrameWork::Instance()'))
    {
      $this->characterEncoding = CMyFrameWork::Instance()->CharacterEncoding();
    }
    else
    {
  ...
  public function GetHTML()
  {
    ...
    $onlyValue 	= isset($this['value']) ? htmlent($this['value']) : null;
    ...
    {
      $Colors = getallcolors();
      $select = "<select id='input1' name='optionvalue'>\n";
      foreach($Colors as $colorname)
      {
        $selected = "";
        if($colorname==$onlyValue)
        {
          $selected = " selected";
        }
        $style = " style='background-color:#" . getcolorvalue($colorname) . "'";
        $select .= "<option value='{$colorname}'{$style}{$selected}>{$colorname}</option>\n";
      }
      $select .= "</select>";
      $html = "<p><label for='input1'>$label</label><br>$select</p>\n";
      return $html;
    }
    else if($type && $this['type'] == 'image')
    {
      $themefiles = readDirectory(getthemepath());
      $select = "<select id='input1' name='optionvalue'>\n";
      foreach($themefiles as $themefile)
      {
      	$imagesize = getimagesize(getthemepath() . "/" . $themefile);
      	if ( ( $imagesize[0] == FaviconLogoWidth ) && ( $imagesize[1] == FaviconLogoHeight ) )
      	{
          $selected = "";
          if($themefile==$onlyValue)
          {
            $selected = " selected";
          }
          $iconfile = replace_extension($themefile, "ico");
          $style = ' style="' . "background-image:url('" . theme_url($iconfile) . "'); background-position:right center; background-repeat: no-repeat;" . '"';
          $select .= "<option value='{$themefile}'{$style}{$selected}>  {$themefile}</option>\n";
        }
      }
      $select .= "</select>";
      $html = "<p><label for='input1'>$label</label><br>$select</p>\n";
    ...
    else if($type && $this['type'] == 'bodyfont')
    {
      return $this->FontSelectionList(getbodyfonts(),$onlyValue,$label);
    }
    else if($type && $this['type'] == 'headerfont')
    {
      return $this->FontSelectionList(getheaderfonts(),$onlyValue,$label);
    }
    else
    {
  ...
  /**
   * Creates a font selection list.
   *
   * @param $fonts array of font names.
   * returns string html font selection list
   */
  private function FontSelectionList($fonts,$value,$label)
  {
    $select = "<select id='input1' name='optionvalue'>\n";
...
class CFormElementOption extends CFormElement
{
  /**
   * Constructor
   *
