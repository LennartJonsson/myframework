...
class CMContent extends CObject implements IHasSQL, ArrayAccess, IModule
{
  ...
  public static function SQL($key=null, $args=null)
  {
    ...
    $queries = array(
      ...
      'select * by id'            => 'SELECT c.*, u.acronym as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE c.id=? AND c.deleted IS null;',
      ...
      'select * by type'          => "SELECT c.*, u.acronym as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE type=? AND c.deleted IS null ORDER BY {$order_by} {$order_order};",
      'select *'                  => 'SELECT c.*, u.acronym as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id AND c.deleted IS null;',
  ...
  public function Manage($action=null)
  {
	...
	try
	{
	  ...
	  $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "This is a demo page, this could be your personal home-page.\n\nMyFrameWork is a PHP-based MVC-inspired Content Management Framework ( Lydia ), watch the making of Lydia at: http://dbwebb.se/lydia/tutorial.", 'plain', $this->user['id']));
	  $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', 'About me', "This is a demo page, this could be your personal about-page.\n\nMyFrameWork is used as a tool to educate in MVC frameworks.", 'plain', $this->user['id']));
