...
class CTextFilter
{
  ...
  public static function Filter($data, $filter)
  {
    ...
    {
      case 'bbcode':
        $data = nl2br(CTextFilter::Bbcode2HTML($data));
        break;

      case 'purify':
        $data = nl2br(CTextFilter::Purify($data));
        break;

      case 'clickable':
        $data = nl2br(CTextFilter::MakeClickable($data));
        break;

      case 'markdown':
        $data = nl2br(CTextFilter::Markdown($data));
        break;

      case 'typography':
        $data = nl2br(CTextFilter::Typographer($data));
        break;

      case 'plain':
      default:
        $data = CTextFilter::Filter($data,'clickable');
        break;
    }
    return $data;
  }
  ...
  {
    include_once(__DIR__.'/PHP_Markdown_1.0.1q/markdown.php');
  ...
  public static function SyntaxHighlightGeSHi($text, $language)
  {
    if(!self::$geshi)
    {
      ...
      //CMyFrameWork::Instance()->views->AddStyle($geshi->get_stylesheet());
    }
...
$href = " href='" . CMyFrameWork::Instance()->CreateUrl('author', $matches[4]) . "'";
...
EOD;
        ...
        case 'BASEURL': return CMyFrameWork::Instance()->request->base_url; break;
