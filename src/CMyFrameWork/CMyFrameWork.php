<?php
/**
 * Main class for MFW, holds everything.
 *
 * @package MyFrameWorkCore
 */
class CMyFrameWork implements ISingleton
{
  ...
  protected function __construct()
  {
    ...
    // Include the site specific config.php and create a ref to $mfw to be used by config.php
    $mfw = &$this;
    require(MFW_SITE_PATH.'/config.php');
    ...
    if( isset($this->config['database'][0]['dsn']) && datadirectorywritable() )
    {
   ...
   * @return CMyFrameWork The instance of this class.
   */
  public static function Instance()
  {
    if(self::$instance == null)
    {
      self::$instance = new CMyFrameWork();
    }
    return self::$instance;
  }

  /**
   * ThemePath, returns theme path.
   */
  public function ThemePath()
  {
    return MFW_INSTALL_PATH . '/' . $this->config['theme']['path'];
  }

  /**
   * CharacterEncoding, returns theme character encoding.
   */
  public function CharacterEncoding()
  {
    return $this->config['character_encoding'];
  }
    ...
    if($controllerExists && $controllerEnabled && $classExists)
    {
      ...
      if($rc->implementsInterface('IController'))
      {
        if($rc->hasMethod($method))
        {
          ...
          {
            die("404. " . get_class() . ' error: Controller method "' . $method . '" not public.');          
          }
        }
        else
        {
          die("404. " . get_class() . ' error: Controller "' . $controller . '" does not contain method "' . $method . '".');
        }
      }
      else
      {
        die('404. ' . get_class() . ' error: Controller "' . $controller . '" does not implement interface IController.');
      }
    }
    else
    { 
  ...
  public function ThemeEngineRender()
  {
    ...
    {
      $parentPath = MFW_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
    }

    // Add stylesheet name to the $mfw->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];

    // Make the theme urls available as part of $mfw
    ...
    $mfw = &$this;
    // First the default MyFrameWork themes/functions.php
    include(MFW_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $mfw->data to own variables and handover to the template file
  ...
  /**
   * Draw HTML for a menu defined in $mfw->config['menus'].
   *
